 ...
 * @author sumit
 */
@WebServlet(name = "PostQueryResponseServlet", urlPatterns = {"/PostQueryResponseServlet"})
public class PostQueryResponseServlet extends HttpServlet {
...
//                               
                    ...
                    if (chkFileOrFormField == true) {                        
                        if (item.getFieldName().equalsIgnoreCase("txtRequestId")) {
                            objBean.setTxtRequestId(Integer.parseInt(item.getString()));
                ...
                if (flag == 1) {
                    ManageExpertAdviceServices objServices=new ManageExpertAdviceServices();
                            ...
                            boolean f=objServices.postQueryResponse(objBean);
                            if(f){
                             response.sendRedirect("module/ManageRequestForAdvice_mp.jsp?msg=Solution posted successfully ");
                            }else{
                                 response.sendRedirect("module/ManageRequestForAdvice_mp.jsp?msg=Solution not  posted");
                            }
                           
                } else {
                    response.sendRedirect("module/ManageRequestForAdvice_mp.jsp");
                }
            } catch (Exception e) {
